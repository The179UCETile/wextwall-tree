...
// data = a function returning the content (actually HTML)
	...
	Vue.component('infobox', {
		...
		<div class="story instant" v-if="tmp[layer].infoboxes && tmp[layer].infoboxes[data]!== undefined && tmp[layer].infoboxes[data].unlocked" v-bind:style="[{'border-color': tmp[layer].color, 'border-radius': player.infoboxes[layer][data] ? 0 : '8px'}, tmp[layer].infoboxes[data].style]">
			<button class="story-title" v-bind:style="[{'background-color': tmp[layer].color}, tmp[layer].infoboxes[data].titleStyle]"
				...
				<span class="story-toggle"><span v-html="player.infoboxes[layer][data] ? "+" : "-""></span></span>
	...
	Vue.component('challenge', {
		...
		<div v-if="tmp[layer].challenges && tmp[layer].challenges[data]!== undefined && tmp[layer].challenges[data].unlocked && !(options.hideChallenges && maxedChallenge(layer, [data]) && !inChallenge(layer, [data]))"
			...
			<button v-bind:class="{ longUpg: true, can: true, [layer]: true }" v-bind:style="{'background-color': tmp[layer].color}" v-on:click="startChallenge(layer, data)"><span v-html="challengeButtonText(layer, data)"></span></button><br><br>
			...
			<span v-else>
				...
				Goal:  <span v-if="tmp[layer].challenges[data].goalDescription" v-html="tmp[layer].challenges[data].goalDescription"></span><span v-else><span v-html="format(tmp[layer].challenges[data].goal)"></span> <span v-html="tmp[layer].challenges[data].currencyDisplayName ? tmp[layer].challenges[data].currencyDisplayName : modInfo.pointsName"></span></span><br>
	...
	Vue.component('upgrade', {
		...
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && tmp[layer].upgrades[data].unlocked" :id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, tooltipBox: true, upg: true, bought: hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
			...
			<span v-else>
				...
				<br><br>Cost: <span v-html="formatWhole(tmp[layer].upgrades[data].cost) "></span> <span v-html="(tmp[layer].upgrades[data].currencyDisplayName ? tmp[layer].upgrades[data].currencyDisplayName : tmp[layer].resource)"></span>
			</span>
	...
	Vue.component('toggle', {
		props: ['layer', 'data'],
		template: `
		<button class="smallUpg can" v-bind:style="{'background-color': tmp[data[0]].color}" v-on:click="toggleAuto(data)"><span v-html="player[data[0]][data[1]]?"ON":"OFF""></span></button>
		`
	})
	...
	Vue.component('main-display', {
		props: ['layer', 'data'],
		template: `
		<div><span v-if="player[layer].points.lt('1e1000')">You have </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}"><span v-html="data ? format(player[layer].points, data) : formatWhole(player[layer].points)"></span></h2> <span v-html="tmp[layer].resource"></span><span v-if="layers[layer].effectDescription">, <span v-html="run(layers[layer].effectDescription, layers[layer])"></span></span><br><br></div>
		`
	})
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			<span v-if="tmp[layer].baseAmount"><br>You have <span v-html="formatWhole(tmp[layer].baseAmount)"></span> <span v-html="tmp[layer].baseResource"></span></span>
			<span v-if="tmp[layer].passiveGeneration"><br>You are gaining <span v-html="format(tmp[layer].resetGain.times(tmp[layer].passiveGeneration))"></span> <span v-html="tmp[layer].resource"></span> per second</span>
			<br><br>
			<span v-if="tmp[layer].showBest">Your best <span v-html="tmp[layer].resource"></span> is <span v-html="formatWhole(player[layer].best)"></span><br></span>
			<span v-if="tmp[layer].showTotal">You have made a total of <span v-html="formatWhole(player[layer].total)"></span> <span v-html="tmp[layer].resource"></span><br></span>
		</div>
		`
	})
	...
	Vue.component('respec-button', {
		...
		template: `
			<div v-if="tmp[layer].buyables && tmp[layer].buyables.respec && !(tmp[layer].buyables.showRespec !== undefined && tmp[layer].buyables.showRespec == false)">
				...
				<button v-on:click="respecBuyables(layer)" v-bind:class="{ longUpg: true, can: player[layer].unlocked, locked: !player[layer].unlocked }" style="margin-right: 18px"><span v-html="tmp[layer].buyables.respecText ? tmp[layer].buyables.respecText : "Respec""></span></button>
			</div>
			`
	})
	...
	Vue.component('master-button', {
		...
		<button v-if="tmp[layer].clickables && tmp[layer].clickables.masterButtonPress && !(tmp[layer].clickables.showMasterButton !== undefined && tmp[layer].clickables.showMasterButton == false)"
			v-on:click="run(tmp[layer].clickables.masterButtonPress, tmp[layer].clickables)" v-bind:class="{ longUpg: true, can: player[layer].unlocked, locked: !player[layer].unlocked }"><span v-html="tmp[layer].clickables.masterButtonText ? tmp[layer].clickables.masterButtonText : "Click me!""></span></button>
	`
	})
	...
	Vue.component('drop-down', {
		...
		template: `
			<select v-model="player[layer][data[0]]">
				<option v-for="item in data[1]" v-bind:value="item"><span v-html="item"></span></option>
			</select>
		`
	})
	...
	Vue.component('sell-one', {
		...
		template: `
			<button v-if="tmp[layer].buyables && tmp[layer].buyables[data].sellOne && !(tmp[layer].buyables[data].canSellOne !== undefined && tmp[layer].buyables[data].canSellOne == false)" v-on:click="run(tmp[layer].buyables[data].sellOne, tmp[layer].buyables[data])"
				v-bind:class="{ longUpg: true, can: player[layer].unlocked, locked: !player[layer].unlocked }"><span v-html="tmp[layer].buyables.sellOneText ? tmp[layer].buyables.sellOneText : "Sell One""></span></button>
	`
	})
	Vue.component('sell-all', {
		...
		template: `
			<button v-if="tmp[layer].buyables && tmp[layer].buyables[data].sellAll && !(tmp[layer].buyables[data].canSellAll !== undefined && tmp[layer].buyables[data].canSellAll == false)" v-on:click="run(tmp[layer].buyables[data].sellAll, tmp[layer].buyables[data])"
				v-bind:class="{ longUpg: true, can: player[layer].unlocked, locked: !player[layer].unlocked }"><span v-html="tmp[layer].buyables.sellAllText ? tmp[layer].buyables.sellAllText : "Sell All""></span></button>
	`
	})
